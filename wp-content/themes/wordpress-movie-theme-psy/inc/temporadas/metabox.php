			...
			jQuery.getJSON(url + input + dtms + imgplus + idioma + apikey, function(tmdbdata) {
				...
				jQuery.each(tmdbdata, function(key, item) {
					...
					if (key == "poster_path") {
						...
						jQuery('#url_image_upload').val("https://image.tmdb.org/t/p/w185"+item);

						<?php if( get_option('dt_api_upload_poster') == 'true') { if (has_post_thumbnail()): else: ?>

						jQuery('#postimagediv p').html("<ul><li><img class='dt_poster_preview' src='https://image.tmdb.org/t/p/w185"+ item +"'/> </li></ul>");
